...
class Fupdater(FupdaterBase):
    def __init__(
        self, config, merger_config, local_models, global_model, merger_ds=None
    ):
        super().__init__(config, merger_config, local_models, global_model, merger_ds)

    def merge_to_global(self, model_coeffs=None, fisher_weights=None, grams=None, **kwargs):
        self.avg_merge(
            self.local_models, self.global_model, model_coeffs, fisher_weights, grams, **kwargs
        )
    ...
    def avg_merge(
        ...
        **kwargs,
        ...
        if grams:
            ...
            print('Start regmean merging ......')
            # import pdb; pdb.set_trace()
            avg_params = self.regmean_mean_params(
                all_params=params, all_grams=grams, model_coeffs=model_coeffs
            )
    ...
    def fisher_weighted_average(self, all_params, model_coeffs, fisher_weights):
        ...
        for n, params in all_params.items():    # [n: 201] para_name_groups
            params = torch.stack(params)  # [N, *]   # [5, 50265, 768]
            fisher = (
            ...
            # logging.info(f'fisher_normalize:{self.merger_config.fisher_normalize}')
            ...
            sum_p = params * fisher * coeff   # [5, 514, 768]*[5, 514, 768]*[5, 1, 1]
            sum_p = sum_p.sum(0)  # [514, 768]

            denom = (fisher * coeff).sum(0)   # [514, 768]  514 or 50265
            # (fisher * coeff).sum(0) / (fisher * coeff).sum(0)   # [514, 768]
            avg_p = sum_p / denom
            avg_params[n] = avg_p
        return avg_params
