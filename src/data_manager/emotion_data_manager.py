...
test_EMOTION_LABELS = ["joy", "anger", "sadness", "disgust", "fear", "surprise", "noemo"]
...
class EmotionDataManager(GLUEDataManager):
    ...
    def normalize_label(self, v):
        # if (v is None) or (v not in test_EMOTION_LABELS):
        if v is None:
            return IGNORE_ID
        else:
    ...
    def get_metrics_func_single(self, model_config):
        ...
        def compute_metrics(p):
            ...
            for label_idx, label_name in enumerate(EMOTION_LABELS):
                # if label_name not in test_EMOTION_LABELS: continue
                mask = p.label_ids[:, label_idx] != IGNORE_ID
                if mask.sum() != 0:
                    y_score, y = preds[mask, label_idx], labels[mask, label_idx]
                    y_hat = y_score >= 0.5   #sum(y_hat)=109, y_hat.shape (20596,)
                    f1 = f1_score(y, y_hat)
            ...
            result["macro_f1"] = np.mean(
                # [v for k, v in result.items() if (k.endswith("_f1") and v!=0.0)]
    ...
    def load_evo_dataset(self, loca_models):
        # datasets = [self.load_dataset(model_config) for _, model_config in vars(model_configs.models).items()]
        # dev_dss = [x[1] for x in datasets]
        evo_ds = ConcatDataset(loc_m.eval_dataset for loc_m in loca_models)
        return evo_ds
